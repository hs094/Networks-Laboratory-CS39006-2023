...
//     printf("PingNetInfo to %s (%s), %d hops max, %d byte packets\n", argv[1], ipaddr, MAX_HOP, PAYLOAD_SIZE);
...
//         exit(EXIT_FAILURE);
//     }
//     int ttl = 1, timeout = time_diff, is_send = 1;
//     fd_set readSockSet;
//     int times = 0;
//     char payload[52];
//     clock_t start_time;
...
//             ip->version = 4;
//             ip->tos = 0; // low delay
//             ip->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + PAYLOAD_SIZE; //https://tools.ietf.org/html/rfc791#page-11
//             ip->id = htons(54322);
//             ip->ttl = ttl;     // hops
//             ip->protocol = 17; // UDP
//             ip->saddr = 0;     //src_addr;
...
int main()
{
    string s = "3       10.118.1.169    0.018 ms        0.010 ms        0.037 ms";
    string e = "10.118.1.169    0.018 ms        0.010 ms        0.037 ms";
    cout << s << endl;
    char t[100];
    memset(t, 0, 100);
    char star[57];
    memset(star, '*', 56);
    sprintf(t, "%d\t%s", 4, star);
    printf("%s\n", t);
    return 0;
}